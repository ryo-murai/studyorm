...
public class SpringJdbcOperations implements DataOperations {
	...
	public void insert(Long customerId, String customerName,
		...
		this.jdbcTemplate.update("INSERT INTO Customer (cust_id, name, email) VALUES (?,?,?)",
				customerId, customerName, customerEmail);
		this.jdbcTemplate.update("INSERT INTO TOrder (item, date, cust_id) VALUES (?,?,?)",
	...
	public int deleteOrdersByNameStartWith(String orderNamePrefix) {
		return this.jdbcTemplate.update(
				"DELETE FROM TOrder WHERE item LIKE ?", 
				orderNamePrefix+"%");
	}
	...
	public int updateOrdersItemOlderThan(Date orderedDate, String newOrderItem) {
		return this.jdbcTemplate.update(
				"UPDATE TOrder SET item = ? WHERE date < ?",
				newOrderItem, orderedDate);
	}
	...
	public String queryCustomerNameByPK(Long id) {
		...
		return this.jdbcTemplate.queryForObject(
				"SELECT c.name FROM Customer c WHERE c.cust_id = ?", 
				params, 
				String.class);
	}
